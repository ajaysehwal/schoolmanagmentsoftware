  ...
  const handlesubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.get(
        `http://localhost:8000/apiadmindata/${formData.admin_email}`
      );
      ...
      if (data.length >= 1) {
        ...
        if (register_token === '') {
          notify('Something went wrong please check again');
        } else {
          if (
            formData.admin_email === '' &&
            formData.admin_name === '' &&
            formData.admin_password === '' &&
            formData.admin_phone == 0
          ) {
            notify('All Input Field are required');
          } else {
            if (formData.admin_password.length < 8) {
              notify('Your password should be minimum 8 characters');
            } else {
              if (formData.admin_password != formData.admin_confirmPassword) {
                notify('Both password should be same.');
              } else {
                postData(formData);
