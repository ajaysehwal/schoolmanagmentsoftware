...
const SchoolCalendar = () => {
 ...
 const [events, setEvents] = useState([]);
  ...
  const postdata=async()=>{
    ...
    try{
      const res=await axios.post(`http://localhost:8000/schoolcalenders`,events[0])
  ...
  const getdata=async(auth:any)=>{
    try{
   const res=await axios.get(`http://localhost:8000/schoolcalenders/${auth}`);
      const eventsdata=[];
    ...
    <>
      ...
      <div style={{padding:'20px'}} className="w-full max-w-full rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <p className='text-2xl font-bold'>School Calendar</p>
              <Button onClick={postdata} style={{display:'flex',alignItems:'center',justifyContent:'right',marginLeft:'auto'}}>Save</Button>
