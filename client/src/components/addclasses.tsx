...
export default function AddClasses() {
  ...
  const postclasses=async(formdata:object )=>{
      setload(true)
      try{
          const res=await axios.post('http://localhost:8000/studentclasses',formdata);
...
const searchbarchange=(e:React.ChangeEvent<HTMLInputElement>)=>{
     console.log(e.target.value);
       if(e.target.value===''){
         ...
         const filtered= classdata.filter(el=>el.class_name.toLowerCase().includes(e.target.value.toLowerCase()));
         setclasses(filtered);
       }
