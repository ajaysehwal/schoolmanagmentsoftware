...
const StudentClasssectionlist = ({data}) => {
  ...
  const deleteclass=async(key:any)=>{
      ...
      try{
       const res=await axios.delete(`http://localhost:8000/studentclasses?key=${key}`);
      ...
      <div className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div className="max-w-full overflow-x-auto">
          <table className="w-full table-auto">
            ...
            <tbody>
              {data[0].length==0?"Section are empty":data[0].map((el)=>(
                    <tr key={el.id}>
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">{el.class}</p>
                    </td>
 ...
 open={open} handler={handleOpen}>
  ...
  export default StudentClasssectionlist;
