...
const Studenthouselist = ({data}) => {
  ...
  const deleteclass=async(key:any)=>{
      ...
      try{
       const res=await axios.delete(`http://localhost:8000/api.studenthouses?key=${key}`);
          ...
          <table className="w-full table-auto">
            ...
            {data[0].map((el)=>(
                    ...
                    <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                      <p className="text-black dark:text-white">{el.house_des}</p>
                    </td>
 ...
 open={open} handler={handleOpen}>
  ...
  export default Studenthouselist;
