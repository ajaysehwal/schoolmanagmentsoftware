...
const CardFive = () => {
  ...
  const getdatacount=async(auth:any)=>{
    try{
      const res=await axios.get(`http://localhost:8000/schoolstudent/${auth}/male`)
        setmale(res.data.length);
    }catch(err){
      return err;
    }
    try{
      const res=await axios.get(`http://localhost:8000/schoolstudent/${auth}/female`)
   setfemale(res.data.length)
    }catch(err){
      return err;
    }
    try{
      const res=await axios.get(`http://localhost:8000/schoolstudent/${auth}/other`)
