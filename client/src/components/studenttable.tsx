...
export default function Studentdetailtable() {
  ...
  const getstudentdata = async (token:any) => {
    try {
      const res = await axios.get(`http://localhost:8000/studentdatabyadmin/${token}`);
      setdata(res.data);
  ...
  const searchbarchange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.value === '') {
      ...
      const filtered = data.filter(el => el.student_name.toLowerCase().includes(e.target.value.toLowerCase()));
      setdata(filtered);
    }
  ...
  return (
    <div>
       ...
       <Link to="/students/admissionform">
         ...
         <Button style={{ display: 'flex', alignItems: 'center', justifyContent: 'right', gap: "5px" }}>
           <svg className="h-6 w-6" viewBox="0 0 24 24" fill="none" stroke="currentColor"
             strokeWidth="2" stroke-linecap="round" stroke-linejoin="round">  <line x1="12" y1="5" x2="12" y2="19" />
             <line x1="5" y1="12" x2="19" y2="12" /></svg>Add Students</Button>
       </Link>
      ...
      <div style={{ marginTop: '10px' }} className="rounded-sm border border-stroke bg-white px-5 pt-6 pb-2.5 shadow-default dark:border-strokedark dark:bg-boxdark sm:px-7.5 xl:pb-1">
        <div style={{ width: '60%', marginBottom: '5px',display:'flex',alignItems:'center',gap:'10px' }}>
          <Input variant="outlined" label="Search student by name" style={{ width: "100%", marginBottom: '10px' }} value={filterval} onInput={(e) => searchbarchange(e)} />
            <select value={filerbyclass} onChange={(e)=>filterbyclass(e)} className='w-full rounded-lg border border-stroke bg-transparent py-2 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary'>
          ...
          <table ref={coverttopdf}  className="w-full table-auto">
            ...
            <tbody>
              ...
              <div style={{textAlign:'center',margin:'auto',width:"100%",display:'grid',alignItems:'center',justifyContent:'center',position:'absolute',padding:'20px'}}>
                 ...
                 <p className='text-2xl font-bold' style={{textAlign:'center',margin:'auto'}}>No Student Found</p>
                 <p style={{textAlign:'center',margin:'auto'}}>Select Classes to view Students.</p>
                 </div>):data?.map((el,i) => (
                <tr  key={el.id}>
                  <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                    <img className="w-20 h-20" src={`http://localhost:8000/${el.student_image}`} alt="" />
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">{el.select_class}</p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">{el.admission_no}</p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">{el.father_name}</p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
