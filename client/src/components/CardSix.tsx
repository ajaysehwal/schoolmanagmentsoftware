...
const CardSix = () => {
  ...
  const getdatacount=async(auth:any)=>{
    setload(true);
    try{
      const res=await axios.get(`http://localhost:8000/schoolteacher/${auth}/male`)
    ...
    try{
       setload(true);
      const res=await axios.get(`http://localhost:8000/schoolteacher/${auth}/female`)
   ...
   setload(false);
    ...
    try{
      ...
      const res=await axios.get(`http://localhost:8000/schoolteacher/${auth}/other`)
