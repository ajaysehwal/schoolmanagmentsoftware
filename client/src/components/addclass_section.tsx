...
export default function AddClasses_section() {
  ...
  const postclasses=async(formdata:object )=>{
      setload(true)
      try{
          const res=await axios.post('http://localhost:8000/studentclasses.section',formdata);
  ...
  const getclasses_section=async(token:any,classes:any)=>{
    ...
    try{
      const res=await axios.get(`http://localhost:8000/studentsection/${classes==''?"I":classes}/${token}`);
        setclasses(res.data);
...
const searchbarchange=(e:React.ChangeEvent<HTMLInputElement>)=>{
     console.log(e.target.value);
       if(e.target.value===''){
         ...
         const filtered= classdata.filter(el=>el.section.toLowerCase().includes(e.target.value.toLowerCase()));
         setclasses(filtered);
       }
...
useEffect(()=>{
    getclasses(verified_token);
    getclasses_section(verified_token,'I');
},[])
      ...
      <div className="grid grid-cols-1 gap-9 sm:grid-cols-2">
            ...
            <form onSubmit={handlesubmit} >
              <div className="p-6.5"> 
                <div className="mb-4.5 flex flex-col gap-6 xl:flex-row">
                  ...
                  <select style={{border:'1px solid rgb(176,190,197)'}} name="class_name" className='w-full rounded-lg border border-stroke bg-transparent py-2 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary' onChange={handlechange} >
        ...
        <div className="flex flex-col gap-9">
          ...
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            ...
            <StudentClasssectionlist data={[classdata,getclasses]}/>
          </div>
        </div>
      </div>
    </div>
