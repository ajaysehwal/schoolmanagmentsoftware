...
const Schooldetails: React.FC = () => {
  const {verified_complete, verifing } = useContext(AuthContext);
  ...
  const register_token = searchParams.get('id');
  // const register_email = searchParams.get('e');

  const verified_token = cookies.get('_VU');
  const verifing_email = cookies.get('_Euhyeadw');
  ...
  const handlesubmit = (e: React.FormEvent<HTMLFormElement>): void => {
    ...
    if(data.user.schoolname==''&&formData.schoolurl===''&&formData.phone==0){
       notify("All input field are mandatory for create new account");
    }else{
       if(formData.phone.length!==10){
              notify('Your number should be 10 character')
       }else{
        postData(formData);
      ...
      const response = await axios.post(
        ...
        formdata
           ...
           if(response.status==200){
             document.location.href=`/register_address?id=${register_token}`
                ...
                >
                  ...
                  <div className="relative">
                    <input
                      ...
                      onChange={handlechange}
                      type="text"
                      placeholder="School url eg. mbschool"
                      className="w-full rounded-lg border border-stroke bg-transparent py-4 pl-6 pr-10 outline-none focus:border-primary focus-visible:shadow-none dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary"
                    />
                  </div>
                </div>
